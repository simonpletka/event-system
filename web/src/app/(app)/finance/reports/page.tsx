        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.35fr_1fr] gap-4 mt-4">
          <div className="card px-4 py-4">
            ...
            <div className="overflow-x-auto">
              <div className="flex items-end gap-2.5 h-[170px] border-b-2 border-ink border-l border-ink/20 pl-1 min-w-[420px]">
              ...
              <div className="flex gap-2.5 pl-1 mt-1 min-w-[420px]">
                {MONTHS.map((m) => (
            ...
            {report.topCategories.map(([cat, value]) => (
              <div key={cat} className="flex flex-col gap-1 py-1.5">
                <div className="flex items-center justify-between text-[13px]">
                  <span>{t.expenseCategories[cat as keyof typeof t.expenseCategories]}</span>
                  <span className="placeholder-text text-[12px]">{formatCurrency(value)}</span>
                </div>
                <div className="h-1.5 bg-accent rounded-full" style={{ width: `${(value / maxCategory) * 100}%`, maxWidth: 160 }} />
              </div>
            ))}
          </div>
        </div>
            ...
            {report.topCategories.map(([cat, value]) => (
              <div key={cat} className="flex flex-col gap-1.5 py-2.5 border-b border-ink/13 last:border-b-0 text-[13px]">
                <div className="flex items-center justify-between">
                  <span className="font-medium">{t.expenseCategories[cat as keyof typeof t.expenseCategories]}</span>
                  <span>{formatCurrency(value)}</span>
                </div>
                <div className="h-2 bg-accent rounded-full" style={{ width: `${(value / maxCategory) * 100}%` }} />
              </div>
            ))}
          </div>
